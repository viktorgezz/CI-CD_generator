...
Определяет, включён ли Python в анализ и какие стадии он добавляет.
"""
...
def enabled(analysis: dict) -> bool:
    return "python" in analysis.get("languages", [])
...
def _has_supported_migration_framework(analysis: dict) -> bool:
    ...
    # тут перечисляем поддерживаемые фреймворки
    supported = {"django", "fastapi", "flask"}
...
def get_stages(analysis: dict, user_settings: dict):
    ...
    enabled_stages = list(PYTHON_STAGES)
